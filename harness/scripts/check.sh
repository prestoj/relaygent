...
if curl -sf --max-time 2 "http://127.0.0.1:${HS_PORT}/health" >/dev/null 2>&1; then
    ...
    # Diagnose WHY computer-use isn't responding
...
if [ -n "$KB_DIR" ]; then
    ...
    if [ -f "$INTENT_FILE" ]; then
        INTENT_LINES=$(grep -cv '^\s*$\|^---\|^#\|^<!--\|^tags:\|^created:\|^updated:\|^title:' "$INTENT_FILE" 2>/dev/null || echo 0)
        if [ "${INTENT_LINES:-0}" -ge 3 ] 2>/dev/null; then ck_ok "Intent" "INTENT.md has content ($INTENT_LINES lines)"
        else ck_warn "Intent" "INTENT.md looks empty — write your priorities in $INTENT_FILE"; fi
    fi
    HANDOFF_FILE="$KB_DIR/HANDOFF.md"
    if [ -f "$HANDOFF_FILE" ]; then
        if grep -qi "MAIN GOAL" "$HANDOFF_FILE" 2>/dev/null; then ck_ok "Handoff" "HANDOFF.md has a main goal"
        else ck_warn "Handoff" "HANDOFF.md missing MAIN GOAL section — agent may lack direction"; fi
    fi
fi

# --- Integrations ---
source "$SCRIPT_DIR/check-integrations.sh"
