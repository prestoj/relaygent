...
export function listSessions() {
	...
	try {
		for (const dir of fs.readdirSync(claudeProjects)) {
			...
			sessions.push({ id: m[0], displayTime: `${m[1]} ${m[2]}:${m[3]}`, size: maxSize, durationMin: st.durationMin, totalTokens: st.totalTokens, toolCalls: st.toolCalls, summary: st.firstText || null });
		}
	} catch { /* ignore */ }
