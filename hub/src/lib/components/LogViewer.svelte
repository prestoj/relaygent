...
<div class="log-search">
	<input type="search" bind:value={search} placeholder="Filter logs..." />
	{#if search}<span class="match-count">{filtered().length} / {lines.length}</span>{/if}
</div>
{#if error}<p class="log-error">{error}</p>{/if}
<div class="log-wrap" bind:this={preEl}>
	{#if lines.length === 0}
		<div class="log-empty">{loading ? 'Loading...' : '(empty)'}</div>
	{:else}
...
<style>
	.log-controls { display: flex; gap: 0.5em; align-items: center; flex-wrap: wrap; margin-bottom: 0.5em; }
	...
	.auto-scroll { display: flex; align-items: center; gap: 0.3em; font-size: 0.85em; color: var(--text-muted); }
	.log-search { display: flex; align-items: center; gap: 0.5em; margin-bottom: 0.5em; }
	.log-search input { flex: 1; padding: 0.35em 0.6em; border: 1px solid var(--border); border-radius: 6px; font-size: 0.85em; background: var(--bg-surface); color: var(--text); }
	.match-count { font-size: 0.78em; color: var(--text-muted); white-space: nowrap; }
	.log-error { color: var(--error); font-size: 0.85em; }
	.log-wrap { background: var(--bg); border: 1px solid var(--border); border-radius: 8px; max-height: 50vh; overflow-y: auto; font-family: monospace; font-size: 0.78em; line-height: 1.5; }
	.log-empty { padding: 1em; color: var(--text-muted); }
