...
async function start() {
  ...
  // Heartbeat: touch cache every 5min; periodic backfill every 30sec (fallback for missing subscriptions)
  setInterval(() => writeCache(readCache()), 5 * 60 * 1000);
  setInterval(async () => { try { await backfill(web); } catch { /* ignore */ } }, 30 * 1000);
